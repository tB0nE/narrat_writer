...
## 1. Game Scaffolding (`CONCEPT_GENERATOR`)
...
## 2. Dynamic Story Continuation (`STORY_CONTINUATION`)
...
## 3. Metadata Refinement (`METADATA_REGENERATOR`)
...
## 4. Asset Description Generation (`ASSET_DESCRIBER`)
...
1. **Centralization**: All prompt templates move to `prompts.py`.
2. **Context Helpers**: Create a `PromptContextBuilder` in `main.py` that gathers dialogue history and metadata into a clean string for the LLM.
3. **Robust Parsing**: Use a unified helper to strip Markdown backticks and ensure JSON/Narrat content is extracted cleanly from AI responses.
