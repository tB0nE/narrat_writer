...
async def rename_asset(game_id: str, req: Dict[str, str]):
    ...
    if category == "characters":
        ...
        for c in meta.characters:
            if c.lower() == old_id.lower():
                ...
                meta_changed = True
                logger.info(f"Metadata Characters List: {c} -> {replacement}")
            else:
    ...
    for field in ["title", "summary", "plot_outline"]:
        ...
        if val:
            ...
            if new_val != val:
                logger.info(f"Metadata Field '{field}': Update detected.")
                setattr(meta, field, new_val)
                meta_changed = True

    if meta_changed:
        save_metadata(game_id, meta)
        logger.info(f"Saved updated metadata.json for {game_id}")
    ...
    if os.path.exists(p):
        ...
        # Apply the smart rename to the entire script file
        new_content = apply_smart_rename(content, old_id, new_id)
        
        if new_content != content:
            with open(p, "w") as f:
                f.write(new_content)
            logger.info(f"Refactored script file: {p}")
        else:
            logger.warning(f"No changes made to script content during refactor!")
